...
La clase debe tener los siguientes métodos:
  ...
  suma el código numérico de cada caracter del input (investigar el método charCodeAt de los strings) y calcula el módulo de ese número total por la cantidad de buckets; 
...
HashTable.prototype.set = function (key, value){
  ...
  let buckets = this.hash(key);
  if (!this.data[buckets]){
    this.data[buckets] = {}
  }
  this.data[buckets][key] = value;
};

HashTable.prototype.get = function (key){
  let buckets = this.hash(key);
  if(this.data[buckets]){
  return this.data[buckets][key];}
  return undefined;
};

HashTable.prototype.hasKey = function (key){
  if(this.data[buckets]){
    return this.data[buckets].hasOwnProperty(key)}
    return false;
};
